...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        // TODO: 实现单向广播的加法
        int m = 0;
        while (m < 4 && shape[m] == others.shape[m]) m++;

        int a[4] = {1, shape[3],shape[3] * shape[2], shape[3] * shape[2] * shape[1]};
        int b[4] = {1, others.shape[3],others.shape[3] * others.shape[2], others.shape[3] * others.shape[2] * others.shape[1]};
